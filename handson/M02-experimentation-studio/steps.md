...
**議論（スライド対応）**: コンテナを使うメリットは何ですか？（再現性・移植性・依存関係の分離）
...
1. 本番でモデルのデプロイを成功させる MLOps プラクティスは？（2 つ）→ **標準リポジトリ / 自動デプロイ**
2. SageMaker がトレーニング・推論のために起動するものは？ → **Docker コンテナ**
3. 学習ロジックだけを差し替え、コンテナは AWS 管理を使う方法は？ → **BYOS（スクリプトモード）**
